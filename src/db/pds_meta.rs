...
// `pb|{host}` -> empty value
pub fn pds_banned_key(host: &str) -> Vec<u8> {
    let mut key = Vec::with_capacity(PDS_BANNED_PREFIX.len() + host.len());
    key.extend_from_slice(PDS_BANNED_PREFIX);
    key.extend_from_slice(host.as_bytes());
    key
}

/// load all banned PDS hosts from the filter keyspace
pub fn load_banned(ks: &Keyspace) -> Result<Vec<SmolStr>> {
    let mut out = Vec::new();
    for guard in ks.prefix(PDS_BANNED_PREFIX) {
        let (k, _) = guard.into_inner().into_diagnostic()?;
        let host = std::str::from_utf8(&k[PDS_BANNED_PREFIX.len()..]).into_diagnostic()?;
        out.push(SmolStr::new(host));
    }
    Ok(out)
}

pub fn set_banned(batch: &mut OwnedWriteBatch, ks: &Keyspace, host: &str) {
    batch.insert(ks, pds_banned_key(host), &[]);
}

pub fn remove_banned(batch: &mut OwnedWriteBatch, ks: &Keyspace, host: &str) {
    batch.remove(ks, pds_banned_key(host));
}
