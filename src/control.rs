...
use crate::db::{
    ...
    load_persisted_firehose_sources, ser_repo_state,
};
...
async fn spawn_firehose_ingestor(
    relay_url: &Url,
    state: &Arc<AppState>,
...
struct FirehoseShared {
    buffer_tx: crate::ingest::BufferTx,
...
pub struct FirehoseHandle {
    state: Arc<AppState>,
    /// set once by [`Hydrant::run`]; `None` means run() has not been called yet.
    shared: Arc<std::sync::OnceLock<FirehoseShared>>,
    /// per-relay running tasks, keyed by url.
    tasks: Arc<scc::HashMap<Url, FirehoseIngestorHandle>>,
    /// set of urls persisted in the database (dynamically added sources).
    persisted: Arc<scc::HashSet<Url>>,
}
